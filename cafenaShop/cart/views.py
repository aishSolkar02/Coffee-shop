...
from cafenaShop.settings import EMAIL_HOST_USER
from django.contrib.auth.decorators import login_required

# Create your views here.
@login_required
def add_to_cart(request,serviceId):
    print("***************",serviceId,"*******")
    print(request.user)
    currentUser=request.user
    cart,created=Cart.objects.get_or_create(user=currentUser)
    request.session["cart_id"]=cart.id
    cartitem,created=CartItem.objects.get_or_create(cart=cart,services=Service.customManager.get(id=serviceId)) #cart from above cart variable
...
def show_cart(request):
    ...
    for cartitem in cartitems:
        total+=cartitem.quantity*cartitem.services.menu_price
...
def payment(request,orderId):
    ...
    for orderitem in orderitems:
        total+=orderitem.quantity*orderitem.services.menu_price
